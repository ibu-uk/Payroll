...
if (isPost()) {
    ...
    if ($sub === 'company') {
        ...
        requireRole('admin');
        $pass = post('password');
        DB::insert('users',['name'=>post('name'),'email'=>post('email'),'password'=>password_hash($pass,PASSWORD_DEFAULT),'role'=>post('role'),'lang'=>post('lang','en')]);
        setFlash('success', t('saved_success'));
...
if (isPost() && post('sub_action') === 'delete_holiday') {
    ...
    setFlash('success', 'Holiday deleted successfully');
    redirect('index.php?page=settings&tab=holidays');
}
...
<ul class="nav nav-tabs mb-4">
  <?php foreach (['company'=>'Company','holidays'=>'Holidays','allowances'=>'Allowance Types','deductions'=>'Deduction Types','employee_deductions'=>'Employee Deductions','users'=>'Users','account'=>'Account'] as $k=>$v): ?>
...
<form method="POST" enctype="multipart/form-data">
  ...
  <div class="row g-3">
      ...
      <div class="card card-modern mb-3">
        ...
        <div class="card-body">
          <div class="row g-3">
            ...
            <div class="col-md-4"><label class="form-label">Currency (EN)</label><input type="text" class="form-control" name="currency" value="<?= h($s['currency']??'KWD') ?>"></div>
            <div class="col-md-4"><label class="form-label">Currency (AR)</label><input type="text" class="form-control" name="currency_ar" dir="rtl" value="<?= h($s['currency_ar']??'د.ك') ?>"></div>
            <div class="col-md-4"><label class="form-label"><?= t('country') ?? 'Country' ?></label><input type="text" class="form-control" name="country" value="<?= h($s['country']??'Kuwait') ?>"></div>
          </div>
        </div>
      </div>
      <div class="card card-modern">
        <div class="card-header-modern"><h5 class="card-title-modern"><i class="fas fa-percent me-2 text-warning"></i>Payroll Configuration</h5></div>
        <div class="card-body">
          <div class="alert alert-info mb-3">
            <i class="fas fa-info-circle me-2"></i>
            <strong>Note:</strong> Working hours are now configured per job title in the "Job Titles" section. Settings below are global defaults.
          </div>
          <div class="row g-3">
            <div class="col-md-4">
              <label class="form-label">Tax Rate (%) <small class="text-muted">(Set to 0 for Kuwait)</small></label>
              <div class="input-group">
                <input type="number" class="form-control" name="tax_rate" step="0.01" min="0" max="100" value="<?= $s['tax_rate']??0 ?>">
                <span class="input-group-text">%</span>
              </div>
              <small class="text-muted">Applied to gross salary</small>
            </div>
            <div class="col-md-4">
              <label class="form-label">Social Insurance Rate (%)</label>
              <div class="input-group">
                <input type="number" class="form-control" name="social_insurance_rate" step="0.01" min="0" max="100" value="<?= $s['social_insurance_rate']??11 ?>">
                <span class="input-group-text">%</span>
              </div>
              <small class="text-muted">Applied to basic salary only</small>
            </div>
            <div class="col-md-4">
              ...
              <small class="text-muted">e.g., 1.25 = 25% extra per hour</small>
            </div>
            <div class="col-md-4">
              <label class="form-label">Holiday Overtime Rate (Multiplier)</label>
              <input type="number" class="form-control" name="holiday_overtime_rate" step="0.01" min="1" value="<?= $s['holiday_overtime_rate']??2.0 ?>">
              <small class="text-muted">e.g., 2.0 = 2x pay for holiday work</small>
            </div>
            <div class="col-md-4">
              ...
              <small class="text-muted">Used for daily rate calculation</small>
            </div>
            <div class="col-md-4">
              <label class="form-label">Default Work Hours / Day</label>
              <input type="number" class="form-control" name="work_hours_per_day" min="4" max="12" value="<?= $s['work_hours_per_day']??8 ?>">
              <small class="text-muted">Fallback if job title not set</small>
            </div>
            <div class="col-md-4">
              ...
              <small class="text-muted">Day of month to process payroll</small>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="col-md-4">
      <div class="card card-modern">
        <div class="card-header-modern"><h5 class="card-title-modern"><i class="fas fa-image me-2"></i>Company Logo</h5></div>
...
<div class="d-flex justify-content-between align-items-center mb-3">
  <div>
    <h5 class="mb-1">Manage Holidays</h5>
    <small class="text-muted">Government, religious, and company holidays</small>
  </div>
  <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#holidayModal"><i class="fas fa-plus me-1"></i>Add Holiday</button>
</div>
<div class="card card-modern">
          ...
          <tr>
            ...
            <td>
              <?php if ($h['is_recurring']): ?>
                <span class="badge bg-success">Every <?= date('F', mktime(0,0,0,$h['recurring_month'],1)) ?> <?= $h['recurring_day'] ?></span>
              <?php else: ?>
            ...
            <td>
              <form method="POST" class="d-inline">
                ...
                <button class="btn btn-sm btn-outline-danger" onclick="return confirm('Are you sure you want to delete this holiday?')"><i class="fas fa-trash"></i></button>
              </form>
            </td>
          </tr>
          <?php endforeach; ?>
          <?php if (empty($holidays)): ?>
          <tr><td colspan="7" class="text-center text-muted py-3">No holidays configured</td></tr>
          <?php endif; ?>
        </tbody>
      </table>
    </div>
    ...
    <div class="modal-content">
      ...
      <form method="POST">
          ...
          <div class="row g-3">
            ...
            <div class="col-md-6">
              ...
              <select class="form-select" name="holiday_type" required>
                ...
                <option value="religious">Religious (Eid, etc.)</option>
            ...
            <div class="col-12">
              <div class="form-check">
                ...
                <label class="form-check-label" for="isRecurring">Recurring Holiday (annual)</label>
              </div>
            </div>
        ...
        <div class="modal-footer">
          <button class="btn btn-primary w-100"><i class="fas fa-save me-1"></i>Save Holiday</button>
        </div>
      </form>
    </div>
  </div>
...
<div class="modal fade" id="allowModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
  <div class="modal-header"><h5 class="modal-title">Allowance Type</h5><button class="btn-close" data-bs-dismiss="modal"></button></div>
  <form method="POST"><input type="hidden" name="_csrf" value="<?= csrf() ?>"><input type="hidden" name="sub_action" value="allow_type"><input type="hidden" name="id" id="allowId" value="0">
    <div class="modal-body">
      <div class="mb-3"><label class="form-label">Name (EN)</label><input type="text" class="form-control" name="name_en" id="allowNameEn" required></div>
      <div class="mb-3"><label class="form-label">Name (AR)</label><input type="text" class="form-control" name="name_ar" id="allowNameAr" dir="rtl"></div>
...
<div class="modal fade" id="dedModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
  <div class="modal-header"><h5 class="modal-title">Deduction Type</h5><button class="btn-close" data-bs-dismiss="modal"></button></div>
  <form method="POST"><input type="hidden" name="_csrf" value="<?= csrf() ?>"><input type="hidden" name="sub_action" value="ded_type"><input type="hidden" name="id" id="dedId" value="0">
    <div class="modal-body">
      <div class="mb-3"><label class="form-label">Name (EN)</label><input type="text" class="form-control" name="name_en" id="dedNameEn" required></div>
      <div class="mb-3"><label class="form-label">Name (AR)</label><input type="text" class="form-control" name="name_ar" id="dedNameAr" dir="rtl"></div>
      <div class="mb-3"><label class="form-label">Calculation Type</label>
...
<div class="d-flex justify-content-between align-items-center mb-3">
  ...
  <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#empDedModal"><i class="fas fa-plus me-1"></i>Add</button>
</div>
<div class="card card-modern">
  <div class="card-body p-0"><table class="table table-hover mb-0">
    <thead class="table-dark"><tr><th>Employee</th><th>Deduction Type</th><th>Amount</th><th>Calc</th><th>Effective</th><th>End</th><th>Actions</th></tr></thead>
    <tbody>
      ...
      <td>
        <div class="btn-group btn-group-sm">
          ...
          <form method="POST" class="d-inline">
            ...
            <button class="btn btn-outline-danger" onclick="return confirm('Remove this deduction?')"><i class="fas fa-trash"></i></button>
          </form>
        </div>
      </td>
    </tr>
    ...
    <tr><td colspan="7" class="text-center text-muted py-3">No employee deductions configured</td></tr>
    <?php endif; ?>
    </tbody>
  </table></div>
</div>
<div class="modal fade" id="empDedModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
  <div class="modal-header"><h5 class="modal-title">Employee Deduction</h5><button class="btn-close" data-bs-dismiss="modal"></button></div>
  <form method="POST"><input type="hidden" name="_csrf" value="<?= csrf() ?>"><input type="hidden" name="sub_action" value="emp_deduction"><input type="hidden" name="id" id="empDedId" value="0">
    <div class="modal-body">
      ...
      <div class="mb-3"><label class="form-label">Amount / Percentage</label><input type="number" step="0.001" class="form-control" name="amount" id="empDedAmount" required></div>
      <div class="mb-3"><label class="form-label">Effective Date</label><input type="date" class="form-control" name="effective_date" id="empDedEffective" required></div>
      <div class="mb-3"><label class="form-label">End Date (optional)</label><input type="date" class="form-control" name="end_date" id="empDedEnd"></div>
    </div>
    <div class="modal-footer"><button class="btn btn-primary w-100"><?= t('save') ?></button></div>
  </form>
</div></div></div>
...
<div class="card card-modern">
  <div class="card-header-modern"><h5 class="card-title-modern"><i class="fas fa-user-shield me-2 text-primary"></i>Change Password</h5></div>
  <div class="card-body">
    <form method="POST">
      <input type="hidden" name="_csrf" value="<?= csrf() ?>">
...
<div class="d-flex justify-content-between align-items-center mb-3">
  ...
  <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#userModal"><i class="fas fa-plus me-1"></i>Add User</button>
</div>
<div class="card card-modern">
  <div class="card-body p-0"><table class="table table-hover mb-0">
    <thead class="table-dark"><tr><th>Name</th><th>Email</th><th>Role</th><th>Language</th><th>Last Login</th><th>Status</th><th>Actions</th></tr></thead>
    <tbody>
      ...
      <td>
        <div class="btn-group btn-group-sm">
          ...
          <form method="POST" class="d-inline">
            ...
            <button class="btn btn-outline-<?= $u['is_active'] ? 'warning' : 'success' ?>" title="<?= $u['is_active'] ? 'Deactivate' : 'Activate' ?>">
              <i class="fas fa-<?= $u['is_active'] ? 'user-slash' : 'user-check' ?>"></i>
            </button>
          </form>
          <button class="btn btn-outline-secondary" onclick="resetPassword(<?= $u['id'] ?>)" title="Reset Password"><i class="fas fa-key"></i></button>
        </div>
      </td>
    </tr>
    <?php endforeach; ?>
    ...
    <div class="modal-body">
      ...
      <div class="mb-3"><label class="form-label">Role</label>
        <select class="form-select" name="role">
          <option value="hr">HR</option><option value="admin">Admin</option><option value="manager">Manager</option><option value="viewer">Viewer</option>
        </select>
      </div>
...
<div class="modal fade" id="editUserModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
  ...
  <form method="POST"><input type="hidden" name="_csrf" value="<?= csrf() ?>"><input type="hidden" name="sub_action" value="edit_user">
    ...
    <div class="modal-body">
      <div class="mb-3"><label class="form-label">Full Name</label><input type="text" class="form-control" name="name" id="editUserName" required></div>
...
<div class="modal fade" id="resetPasswordModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
  ...
  <form method="POST"><input type="hidden" name="_csrf" value="<?= csrf() ?>"><input type="hidden" name="sub_action" value="reset_password">
    ...
    <div class="modal-footer"><button class="btn btn-primary w-100">Reset Password</button></div>
  </form>
</div></div></div>
